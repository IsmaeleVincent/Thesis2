...
G=2*pi/LAM
bcr1=7.640#8.783 #7.915 #scattering lenght x density
bcr2=1.817#0.826 #1.361
bcr3=1.717#1.688
n_0 =1.
phi=0
phi1=0
d0=78
krange=[0,1,2,3,4,5]#range(len(foldername))#[1]#[0]#[0,2,3,4,5]#range(7,len(foldername))#[0,2,3,4,5] #np.arange(len(foldername))#
...
def process_fit(k):
    ...
    fit_res =  np.loadtxt(data_analysis1+foldername[k]+'_fit_results_bcr3.mpa',skiprows=1)
    ...
    if (fitting):
        if k==1 or k>5 or 0==0:
            ...
            # P0[3]=0
            # P0[4]=0
            B=([2e-3, 1e-4, 0.0001, -0.0005/rad, -5],[3.8e-3, 1e-3, 0.002, 0.0005/rad, 5])
        # if k<1 or (k>1 and k<6):
        #     P0[0]=3.4e-3
        #     P0[1]=0.0001
        #     P0[2]=0.0001
...
if (save_fit_tot):
    ...
    with open(sorted_fold_path+'tot_fit_results_bcr_fix.mpa', 'w') as f:
          np.savetxt(f,tot_res, header="tilt bcr1 bcr2 mu sigma tau x0 d", fmt="%.2f "+"%.6f "*len(fit_res[0,:]))
    with open(sorted_fold_path+'tot_fit_covariances_bcr_fix.mpa', 'w') as f:
