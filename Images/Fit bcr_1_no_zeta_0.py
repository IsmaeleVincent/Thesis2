...
B0i0=[7, 1.4e-3, 5e-6, 0.5, -0.0005/rad]
B0f0=[15, 4e-3, 1.1e-3, 6, 0.0005/rad]
...
for group in [2]: #0 for Juergen, 1 for Martin, 2 for Christian, 3 for all
    ...
    #         fig, ax = plt.subplots(len(fit_res[0,1:]),figsize=(7,1.5),sharex=True)
    ...
    if param_ev_plot:
        if group!=2:
            ...
            plt.xticks(range(len(fit_par[:,0])),fit_par[:,0])
            for i in [0]:# range(len(fit_res[0,1:])):
                ax.set_ylabel(p_name[i]+p_units[i])
                ax.errorbar(np.arange(len(fit_par[:,i+1])),fit_par[:,i+1], yerr=fit_cov[:,i+1], color="k")
