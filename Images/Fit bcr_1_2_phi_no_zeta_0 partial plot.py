...
measur_groups=[[0,2,3,4,5],[9,10,11,12],[0,1,6], range(13)]
...
for group in [2]: #0 for Juergen, 1 for Martin, 2 for Christian, 3 for all
    ...
    if (plotting):
        for k in krange:
            ...
            if(extended_plot):
                ...
                fig = plt.figure(figsize=(7,1))#constrained_layout=True
                gs_t = GridSpec(1, 1, figure=fig,hspace=0)
                ax = [fig.add_subplot(gs_t[0,:])]
                      #fig.add_subplot(gs_t[1,:])]
                      #fig.add_subplot(gs_t[2,:])]
                ...
                ax[0].plot([],[],"-k", label="Fit")
                ax[0].errorbar([],[], fmt="vk", yerr= [], label="Data")
                # ax[0].set_yticks([round(np.amin(diff_eff[:,6]),2),round(np.amax(diff_eff[:,6]),2)])
                ax[0].legend()
                for i in range(2,3):
                    if i<3:
                        ax[i-2].errorbar(diff_eff[:,0]*rad,diff_eff[:,6-2*i], fmt="vk", yerr=diff_eff[:,7-2*i], label="Data (-"+str(i)+")")
                        ax[i-2].set_ylabel("Order $\pm$"+str(i))
                        ax[i-2].set_yticks([round(np.amin(diff_eff[:,6-2*i]),2),round(np.amax(diff_eff[:,6-2*i]),2)])
                        ax[i-2].errorbar(diff_eff[:,0]*rad,diff_eff[:,6+2*i], fmt="v",  color = (0.8,0,0),  yerr=diff_eff[:,7+2*i],  label="Data (-"+str(i)+")")
                    ax[i-2].plot(thx,eta[n_diff-i,:],"-k", label="Fit (-"+str(i)+")")
                    ax[i-2].plot(thx,eta[n_diff+i,:],"-",color = (0.8,0,0), label="Fit (+"+str(i)+")")   
                    # ax[i].legend()
                ax[-1].set_xlabel("$\\theta$ (rad)")
                fig.text(0.03, 0.5, 'Diff. efficiency', va='center', rotation='vertical', fontsize=11)
            plt.savefig("Fit_"+str(tilt[k])+"_deg_"+fit_name+"_part1.pdf", format="pdf",bbox_inches="tight")
            now2=datetime.now()
    ...
    if param_ev_plot:
        if group!=2:
            ...
            fig, ax = plt.subplots(len(fit_res[0,1:]),figsize=(8,5),sharex=True)
            ...
            for i in range(len(fit_res[0,1:])):
                ...
                ax[i].errorbar(np.arange(len(fit_par[:,i+1])),fit_par[:,i+1], yerr=fit_cov[:,i+1])
        ...
        for k in krange:
            ...
            if group==3:   
                ...
                if k==1:
                    ...
                    c3=1
            ax.plot(x*1e3,a1/np.amax(a1),"-", color=(round(np.random.rand(),3)*c1,round(np.random.rand(),3)*c2,round(np.random.rand(),3)*c3), label= "$\zeta = $"+str(tilt[k])+" deg")
