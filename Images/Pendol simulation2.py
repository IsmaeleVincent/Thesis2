...
for zeta in tilt:
    ...
    d=zeta#d0/np.cos((zeta*rad))
    # pendol[k,0] = d
    th=np.linspace(-0.02,-0.01,1000)#[-0.00893939]#[x[0]*rad-3*div,*x*rad,x[-1]*rad+3*div]
    S=np.zeros((2*n_diff+1,len(th)),dtype=complex)
    eta=S.copy().real
    eta_aus=eta.copy()
    sum_diff = np.zeros(len(th))
    ...
    n_2 = bcr2*2*pi/b**2
    n_3 = bcr3*2*pi/b**2
    for t in range(len(th)):#range(len(th2)):
        A = np.zeros((2*n_diff+1,2*n_diff+1), dtype=complex)
        for i in range(len(A[0])):
            A[i][i]=-dq_j(th[t],i-n_diff,G,b)
            if(i+1<len(A[0])):
    ...
    ax[k].set_xticks([1.2,1.5,1.8])
ax[2].set_xlabel("$\\theta$ (rad$\cdot10^{-2}$)")
ax[1].set_ylabel("Diff. efficiency")
ax[1].set_ylabel("Diff. efficiency")
plt.savefig('Pendel_simulation2.eps', format='pdf',bbox_inches='tight')
