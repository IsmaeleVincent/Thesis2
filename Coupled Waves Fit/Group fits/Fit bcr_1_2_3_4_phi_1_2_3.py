...
# -*- coding: utf-8 -*-
"""
Created on Mon Sep  5 16:03:06 2022
...
allfits_plots= [alldata_analysis + "All fits plots/Juergen/bcr_1_2_3_4_phi_1_2_3/",
                alldata_analysis + "All fits plots/Martin/bcr_1_2_3_4_phi_1_2_3/",
                alldata_analysis + "All fits plots/Christian/bcr_1_2_3_4_phi_1_2_3/",
                alldata_analysis + "All fits plots/All groups/bcr_1_2_3_4_phi_1_2_3/"]
...
for group in [0,1,2,3]: #0 for Juergen, 1 for Martin, 2 for Christian, 3 for all
    ...
    def process_fit(k):
        ...
        def fit_func(x, bcr1, bcr2, bcr3, bcr4, mu1, sigma, tau, x00, zeta0, phi, phi1, phi2):
            x=diff_eff[:,0]+x00
                ...
                for t in range(len(th)):
                    ...
                    for i in range(len(A[0])):
                        ...
                        if(i+4<len(A[0]) and bcr4!=0):
                            A[i][i+4]=b**2*n_0*n_4*np.exp(-1j*phi2)/(2*k_jz(th[t],i-n_diff,G,b))
                            A[i+4][i]=b**2*n_0*n_4*np.exp(1j*phi2)/(2*k_jz(th[t],i-n_diff,G,b))
                    A=-1j*A
                    w,v = np.linalg.eig(A)
        ...
        if (fitting):
            ...
            P0[10]=0
            P0[11]=0
            Bi_groups=[[5, 0, 0, 0, 2.5e-3, 2e-5, 0.1, -0.0005/rad, -2, 0, 0, 0],
                      [5, 0, 0, 0, 2.5e-3, 2e-5, 0.1, -0.0005/rad, -2, 0, 0, 0],
                      [5, 0, 0, 0, 2.5e-3, 2e-5, 0.1, -0.0005/rad, -2, 0, 0, 0],
                      [5, 0, 0, 0, 2.5e-3, 2e-5, 0.1, -0.0005/rad, -2, 0, 0, 0]]
            Bf_groups=[[10, 5, 5, 5, 4e-3, 1.5e-3, 10, 0.0005/rad, 2,2,2,2],
                      [10, 5, 5, 5, 4e-3, 1.5e-3, 10, 0.0005/rad, 2,2,2,2],
                      [10, 5, 5, 5, 4e-3, 1.5e-3, 10, 0.0005/rad, 2,2,2,2],
                      [10, 5, 5, 5, 4e-3, 1.5e-3, 10, 0.0005/rad, 2,2,2,2]]
            ...
            if (save_fit_res):
                with open(data_analysis+foldername[k]+'_fit_results_bcr_1_2_3_4_phi_1_2_3.mpa', 'w') as f:
    ...
    if (plotting):
        for k in krange:
            ...
            data_analysis = sorted_fold_path+foldername[k]+"/Data Analysis/"
            diff_eff =  np.loadtxt(data_analysis+foldername[k]+'_diff_eff_new.mpa',skiprows=1)
            fit_res =  np.loadtxt(data_analysis+foldername[k]+'_fit_results_bcr_1_2_3_4_phi_1_2_3.mpa',skiprows=1)
            diff_eff[:,3::2]=diff_eff[:,2::2]**0.5
            diff_eff_aus=diff_eff[:,2::2].copy()
            diff_eff_aus_err=diff_eff[:,3::2].copy()
            diff_eff_aus[diff_eff_aus==0]=1
            ...
            def plot_func(x, bcr1, bcr2, bcr3, bcr4, mu1, sigma, tau, x00,zeta0, phi, phi1, phi2):
                x=diff_eff[:,0]+x00
                phi=phi*pi
                phi1=phi1*pi
                phi2=phi2*pi
            ...
            p_name=["$(b_c \\rho)_1$","$(b_c \\rho)_2$","$(b_c \\rho)_3$","$(b_c \\rho)_4$", "$\mu$", "$\sigma$","$\\tau$", "$x_0$","$\zeta_0$","$\phi$", "$\phi_1$", "$\phi_2$"]
            p_units=[" $1/\mu m^2$"," $1/\mu m^2$"," $1/\mu m^2$"," $1/\mu m^2$"," nm", " nm", "", " deg", "  deg", " $\pi$", " $\pi$", " $\pi$"]
            text = "Fit results"
            if(extended_plot):
                p=fit_res[0]
            ...
            plt.savefig(alldiff_eff_fit[group]+str(tilt[k])+'deg_fit_bcr_1_2_3_4_phi_1_2_3_'+str(tilt[k])+'_deg.png', format='png',bbox_inches='tight')
            if close_fig:
                plt.close(fig)
            now2=datetime.now()
    ...
    tot_res = np.zeros((nmeas_groups[group], 13))
    ...
    with open(allfits_plots[group]+'tot_fit_results_bcr_1_2_3_4_phi_1_2_3.mpa', 'w') as f:
          np.savetxt(f,tot_res, header="tilt bcr1 bcr2 mu sigma tau x0 d", fmt="%.2f "+"%.6f "*len(fit_res[0,:]))
    with open(allfits_plots[group]+'tot_fit_covariances_bcr_1_2_3_4_phi_1_2_3.mpa', 'w') as f:
          np.savetxt(f,tot_cov, header="tilt bcr1 bcr2 mu sigma tau x0 d", fmt="%.2f "+"%.6f "*len(fit_res[0,:]))
    # if group==3:
    #    with open(sorted_fold_path+'Total results/tot_fit_results_bcr_1_2_3_4_phi_1_2_3.mpa', 'w') as f:
    #          np.savetxt(f,tot_res, header="tilt bcr1 bcr2 mu sigma tau x0 d", fmt="%.2f "+"%.6f "*len(fit_res[0,:]))
    #    with open(sorted_fold_path+'Total results/tot_fit_covariances_bcr_1_2_3_4_phi_1_2_3.mpa', 'w') as f:
    #          np.savetxt(f,tot_cov, header="tilt bcr1 bcr2 mu sigma tau x0 d", fmt="%.2f "+"%.6f "*len(fit_res[0,:])) 
    ...
    """
    if group!=2:
        fit_res =  np.loadtxt(allfits_plots[group]+'tot_fit_results_bcr_1_2_3_4_phi_1_2_3.mpa',skiprows=1)
        fit_cov =  np.loadtxt(allfits_plots[group]+'tot_fit_covariances_bcr_1_2_3_4_phi_1_2_3.mpa',skiprows=1)
        fig, ax = plt.subplots(len(fit_res[0,1:]),figsize=(10,10),sharex=True)
        #plt.subplots_adjust(hspace=0.5)
        plt.xticks(range(len(fit_res[:,0])),fit_res[:,0]) 
        
        title=["$(b_c \\rho)_1$","$(b_c \\rho)_2$","$(b_c \\rho)_3$","$(b_c \\rho)_4$", "$\mu$", "$\sigma$","$\\tau$", "$x_0$","$\zeta_0$","$\phi$", "$\phi_1$", "$\phi_2$"]
    ...
    fit_res_meas =  np.loadtxt(allfits_plots[group]+'tot_fit_results_bcr_1_2_3_4_phi_1_2_3.mpa',skiprows=1)
    fit_cov_meas = np.loadtxt(allfits_plots[group]+'tot_fit_covariances_bcr_1_2_3_4_phi_1_2_3.mpa',skiprows=1)
    n_plots=4
    if group==2:
        n_plots=2
    kaus=-1
