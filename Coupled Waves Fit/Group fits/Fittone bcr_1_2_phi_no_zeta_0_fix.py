...
fit_name="bcr_1_2_phi_no_zeta_0_fix"
fit_name_aus="bcr_1_2_phi_no_zeta_0"
p_name=["$\mu$", "$\sigma$","$\\tau$", "$x_0$"]
p_units=[" nm", " nm", "", " deg"]
...
bcr1=10.07 #scattering lenght x density
bcr2=3.60
bcr3=0.
n_0 =1.
phi=0.54*pi #phase shift bcr2
phi1=0 #phase shift bcr3
d0=78 #sample thickness

B0i0=[1.2e-3, 5e-6, 0.5, -0.0008/rad]
B0f0=[4e-3, 1.4e-3, 5, 0.0008/rad]
...
measur_groups=[[0,2,3,4],[6,7,8,9,10,11],[1], range(13)]

for group in [1,2,3]: #0 for Juergen, 1 for Martin, 2 for Christian, 3 for all
    tilt=[0,40,48,61,69,71,79,80,81,77.88,76.76,75.64,74.52]
    ...
    def process_fit(k):
        ...
        diff_eff = diff_eff[diff_eff[:,0]*rad>=-0.0025]
        diff_eff_fit=diff_eff[:,2::2].copy()
        diff_eff_err=diff_eff[:,3::2].copy()
        def fit_func(x, mu1, sigma, tau, x00):
            x=diff_eff[:,0]+x00
            ...
            return aaa
        P0= fit_res[0,2:-1]  #np.zeros(4) #fit_res[0]  # [*fit_res[0],0  # fit_res[0] # [*fit_res[0,:-1],0,0]  # fit_res[0] #  [8, 2,0, 2.01e-3, pi,0, 75, 1000, 0.0004] #    [5,0,2.6e-3] # 
        # P0[0]=8
        # P0[1]=0
        # P0[2]=mu0
        # P0[3]=sigma0
    ...
    if (plotting):
        for k in krange:
            ...
            p=fit_res[0]
            def plot_func(x, mu1, sigma, tau, x00):
                x=diff_eff[:,0]+x00
                d=d0/np.cos((tilt[k])*rad)
                wl=exponnorm.ppf(np.arange(0.01,0.99,wlp),K=tau, loc=mu1, scale=sigma)
                a=rho(wl,tau, mu1, sigma)/sum(rho(wl,tau, mu1, sigma))
    ...
    tot_res = np.zeros((2*nmeas_groups[group], 5))
    kaus=-2
    for k in krange:
        kaus+=2
        #print(foldername[k])
