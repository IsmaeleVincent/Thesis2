...
Bi_groups=[[5, 0, 0, 1.4e-3, 2e-5, 0.5, -0.0005/rad, -2],
          [5, 0, 0, 1.4e-3, 2e-5, 0.5, -0.0005/rad, -2],
          [5, 0, 0, 1.4e-3, 2e-5, 0.5, -0.0005/rad, -2],
          [5, 0, 0, 1.4e-3, 2e-5, 0.5, -0.0005/rad, -2]]
Bf_groups=[[13, 7,2 , 4e-3, 0.65e-3, 10, 0.0005/rad, 2],
          [13, 7,2 , 4e-3, 0.65e-3, 10, 0.0005/rad, 2],
          [13, 7,2 , 4e-3, 0.65e-3, 10, 0.0005/rad, 2],
          [13, 7,2 , 4e-3, 0.65e-3, 10, 0.0005/rad, 2]]

measur_groups=[[0,2,3,4,5,6],[6,7,8,9,10,11,12],[1], range(13)]

for group in [0,2]: #0 for Juergen, 1 for Martin, 2 for Christian, 3 for all
    now=datetime.now()
    ...
    if fitting:
        ...
        for kf in krange:
            ...
            if kaus>0:
                P0=np.append(P0, Paus[3:])
                BI=np.append(BI,BIaus[3:])
                BF=np.append(BF,BFaus[3:])
            kaus+=1
        # print(len(P0))
        # plt.plot(FF)
        # print(len(P0))
    ...
    if (plotting):
        for k in krange:
            ...
            p=fit_res[0]
            print(p)
            def plot_func(x, bcr1, bcr2, phi, mu1, sigma, tau, x00,zeta0):
                x=diff_eff[:,0]+x00
                phi*pi
                d=d0/np.cos((tilt[k]+zeta0)*rad)
                wl=exponnorm.ppf(np.arange(0.01,0.99,wlp),K=tau, loc=mu1, scale=sigma)
