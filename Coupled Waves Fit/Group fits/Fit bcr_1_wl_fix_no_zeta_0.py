...
fit_name="bcr_1_wl_fix_no_zeta_0"
...
for group in [3]: #0 for Juergen, 1 for Martin, 2 for Christian, 3 for all
    ...
    def process_fit(k):
        ...
        fit_res =  np.loadtxt(data_analysis+foldername[k]+"_fit_results_"+"bcr_1_wl_fix"+".mpa",skiprows=1)
        # diff_eff = diff_eff[diff_eff[:,0]<=0]
        diff_eff_fit=diff_eff[:,2::2]
        diff_eff_err=diff_eff[:,3::2]
        def fit_func(x, bcr1, x00):
            x=diff_eff[:,0]+x00
            ...
            return aaa
        P0= fit_res[0] # np.zeros(3) #  [*fit_res[0],0  # fit_res[0] # [*fit_res[0,:-1],0,0]  # fit_res[0] #  [8, 2,0, 2.01e-3, pi,0, 75, 1000, 0.0004] #    [5,0,2.6e-3] # 
        if (fitting):
            # P0[0]=8
            # P0[1]=0
            # P0[2]=0
