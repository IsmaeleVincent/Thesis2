...
# -*- coding: utf-8 -*-
"""
Created on Tue Sep  6 19:06:49 2022
...
allfits_plots= [alldata_analysis + "All fits plots/Juergen/bcr_1_2_3_phi_1_2_no_zeta0/",
                alldata_analysis + "All fits plots/Martin/bcr_1_2_3_phi_1_2_no_zeta0/",
                alldata_analysis + "All fits plots/Christian/bcr_1_2_3_phi_1_2_no_zeta0/",
                alldata_analysis + "All fits plots/All groups/bcr_1_2_3_phi_1_2_no_zeta0/"]
...
for group in [0,1,2,3]:#0 for Juergen, 1 for Martin, 2 for Christian, 3 for all
    ...
    def process_fit(k):
        ...
        def fit_func(x, bcr1, bcr2, bcr3, mu1, sigma, tau, x00, phi, phi1):
            x=diff_eff[:,0]+x00
        ...
        if (fitting):
            ...
            P0[7]=1
            P0[8]=0
            Bi_groups=[[5, 0, 0, 2.5e-3, 2e-5, 0.1, -0.0005/rad, 0, 0],
                      [5, 0, 0, 2.5e-3, 2e-5, 0.1, -0.0005/rad,  0, 0],
                      [5, 0, 0, 2.5e-3, 2e-5, 0.1, -0.0005/rad,  0, 0],
                      [5, 0, 0, 2.5e-3, 2e-5, 0.1, -0.0005/rad,  0, 0]]
            Bf_groups=[[10, 5, 5, 4e-3, 1.5e-3, 10, 0.0005/rad, 2,2],
                      [10, 5, 5, 4e-3, 1.5e-3, 10, 0.0005/rad, 2,2],
                      [10, 5, 5, 4e-3, 1.5e-3, 10, 0.0005/rad, 2,2],
                      [10, 5, 5, 4e-3, 1.5e-3, 10, 0.0005/rad, 2,2]]
            ...
            if (save_fit_res):
                with open(data_analysis+foldername[k]+'_fit_results_bcr_1_2_3_phi_1_2_no_zeta0.mpa', 'w') as f:
    ...
    if (plotting):
        for k in krange:
            ...
            data_analysis = sorted_fold_path+foldername[k]+"/Data Analysis/"
            diff_eff =  np.loadtxt(data_analysis+foldername[k]+'_diff_eff_new.mpa',skiprows=1)
            fit_res =  np.loadtxt(data_analysis+foldername[k]+'_fit_results_bcr_1_2_3_phi_1_2_no_zeta0.mpa',skiprows=1)
            diff_eff[:,3::2]=diff_eff[:,2::2]**0.5
            diff_eff_aus=diff_eff[:,2::2].copy()
            diff_eff_aus_err=diff_eff[:,3::2].copy()
            diff_eff_aus[diff_eff_aus==0]=1
            ...
            def plot_func(x, bcr1, bcr2, bcr3, mu1, sigma, tau, x00, phi, phi1):
                x=diff_eff[:,0]+x00
                phi=phi*pi
                phi1=phi1*pi
                d=d0/np.cos((tilt[k])*rad)
            ...
            p_name=["$(b_c \\rho)_1$","$(b_c \\rho)_2$","$(b_c \\rho)_3$", "$\mu$", "$\sigma$","$\\tau$", "$x_0$","$\phi$", "$\phi_1$"]
            p_units=[" $1/\mu m^2$"," $1/\mu m^2$"," $1/\mu m^2$"," nm", " nm", "", " deg", " $\pi$", " $\pi$"]
            text = "Fit results"
            if(extended_plot):
                p=fit_res[0]
            ...
            plt.savefig(alldiff_eff_fit[group]+str(tilt[k])+'deg_fit_bcr_1_2_3_phi_1_2_no_zeta0_'+str(tilt[k])+'_deg.png', format='png',bbox_inches='tight')
            if close_fig:
                plt.close(fig)
            now2=datetime.now()
    ...
    tot_res = np.zeros((nmeas_groups[group], 11))
    ...
    with open(allfits_plots[group]+'tot_fit_results_bcr_1_2_3_phi_1_2_no_zeta0.mpa', 'w') as f:
          np.savetxt(f,tot_res, header="tilt bcr1 bcr2 mu sigma tau x0 d", fmt="%.2f "+"%.6f "*len(fit_res[0,:]))
    with open(allfits_plots[group]+'tot_fit_covariances_bcr_1_2_3_phi_1_2_no_zeta0.mpa', 'w') as f:
    ...
    if param_ev_plot:
        if group!=2:
            fit_res =  np.loadtxt(allfits_plots[group]+'tot_fit_results_bcr_1_2_3_phi_1_2_no_zeta0.mpa',skiprows=1)
            fit_cov =  np.loadtxt(allfits_plots[group]+'tot_fit_covariances_bcr_1_2_3_phi_1_2_no_zeta0.mpa',skiprows=1)
            ...
            title=["$(b_c \\rho)_1$","$(b_c \\rho)_2$","$(b_c \\rho)_3$", "$\mu$", "$\sigma$","$\\tau$", "$x_0$", "$\phi$", "$\phi_1$"]
            ...
            plt.savefig(allfits_plots[group]+'Param_evolution_bcr_1_2_3_phi_1_2_no_zeta0.png', format='png',bbox_inches='tight')
            if close_fig:
                plt.close(fig)
    """
    """
    ...
    if (wl_plot):
        ...
        fit_res_meas =  np.loadtxt(allfits_plots[group]+'tot_fit_results_bcr_1_2_3_phi_1_2_no_zeta0.mpa',skiprows=1)
        fit_cov_meas = np.loadtxt(allfits_plots[group]+'tot_fit_covariances_bcr_1_2_3_phi_1_2_no_zeta0.mpa',skiprows=1)
        n_plots=4
        if group==2:
            n_plots=2
        kaus=-1
