...
# -*- coding: utf-8 -*-
"""
Created on Mon Sep  5 15:18:03 2022
...
allfits_plots= [alldata_analysis + "All fits plots/Juergen/bcr_1_2_no_div/",
                alldata_analysis + "All fits plots/Martin/bcr_1_2_no_div/",
                alldata_analysis + "All fits plots/Christian/bcr_1_2_no_div/",
                alldata_analysis + "All fits plots/All groups/bcr_1_2_no_div/"]
...
for group in [0,1,2,3]:#0 for Juergen, 1 for Martin, 2 for Christian, 3 for all
    ...
    def process_fit(k):
        ...
        # fit_res =  np.loadtxt(data_analysis+foldername[k]+'_fit_results_bcr_1_2_no_div.mpa',skiprows=1)
    ...
    with open(allfits_plots[group]+'tot_fit_results_bcr_1_2_no_div.mpa', 'w') as f:
          np.savetxt(f,tot_res, header="tilt bcr1 bcr2 mu sigma tau x0 d", fmt="%.2f "+"%.6f "*len(fit_res[0,:]))
    with open(allfits_plots[group]+'tot_fit_covariances_bcr_1_2_no_div.mpa', 'w') as f:
          np.savetxt(f,tot_cov, header="tilt bcr1 bcr2 mu sigma tau x0 d", fmt="%.2f "+"%.6f "*len(fit_res[0,:]))
    if group==3:
       with open(sorted_fold_path+'Total results/tot_fit_results_bcr_1_2_no_div.mpa', 'w') as f:
             np.savetxt(f,tot_res, header="tilt bcr1 bcr2 mu sigma tau x0 d", fmt="%.2f "+"%.6f "*len(fit_res[0,:]))
       with open(sorted_fold_path+'Total results/tot_fit_covariances_bcr_1_2_no_div.mpa', 'w') as f:
             np.savetxt(f,tot_cov, header="tilt bcr1 bcr2 mu sigma tau x0 d", fmt="%.2f "+"%.6f "*len(fit_res[0,:])) 
    ...
    if param_ev_plot:
        if group!=2:
            fit_res =  np.loadtxt(allfits_plots[group]+'tot_fit_results_bcr_1_2_no_div.mpa',skiprows=1)
            fit_cov =  np.loadtxt(allfits_plots[group]+'tot_fit_covariances_bcr_1_2_no_div.mpa',skiprows=1)
            ...
            plt.savefig(allfits_plots[group]+'Param_evolution_bcr_1_2_no_div.png', format='png',bbox_inches='tight')
            if close_fig:
                plt.close(fig)
    """
    """
    ...
    if wl_plot:
        ...
        fit_res_meas =  np.loadtxt(allfits_plots[group]+'tot_fit_results_bcr_1_2_no_div.mpa',skiprows=1)
        fit_cov_meas = np.loadtxt(allfits_plots[group]+'tot_fit_covariances_bcr_1_2_no_div.mpa',skiprows=1)
        n_plots=4
        if group==2:
            n_plots=2
        kaus=-1
