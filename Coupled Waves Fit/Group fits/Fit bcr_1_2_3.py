...
Bi=[5, 0, 0, 2.5e-3, 2e-5, 0.1, -0.0005/rad, -2]
Bf=[10, 5, 5, 4e-3, 1.5e-3, 10, 0.0005/rad, 2]
            ...
            if(extended_plot):
                ...
                for i in range(len(p)):
                    if not i%3:
                        ...
                        text+= "\t"
                    text+= p_name[i] + "=" + str("%.3f" % (fit_res[0,i],)) + "$\pm$" + str("%.3f" % (fit_res[1,i],)) + p_units[i]
                ax[-1].text(diff_eff[0,0]*rad,-np.amax(diff_eff[:,2])*2/3, text,  bbox=dict(boxstyle="square", ec=(0, 0, 0), fc=(1,1,1)))
                #ax[1].text( diff_eff[0,0]*rad,np.amax(diff_eff_fit[3,:]), "p value="+str("%.3f" % (chi[1],)),  bbox=dict(boxstyle="square", ec=(0, 0, 0), fc=(1,1,1)))
            plt.savefig(alldiff_eff_fit[group]+"fit_"+fit_name+"_"+str(tilt[k])+"_deg.png", format="png",bbox_inches="tight")
            if close_fig:
                plt.close(fig)
            now2=datetime.now()
