...
group=2 #0 for Juergen, 1 for Martin, 2 for Christian, 3 for all
"""
...
for k in krange:
    ...
    if kaus%n_plots==0:
        if kaus>0:
            plt.savefig(allwl_plots[group]+str(tilt[k-4])+"-"+str(tilt[k-1])+'deg_wl.png', format='png',bbox_inches='tight')
            # plt.close(fig)
