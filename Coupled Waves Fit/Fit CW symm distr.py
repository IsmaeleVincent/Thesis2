...
def rho(x,x0,sigma,tau):
    g=skewnorm(loc=x0,a=tau,scale=sigma)
    return g.pdf(x)
...
d0=78
tau=0
krange=[0,2,3,4,5] #[0]# [0,2,3,4,5] #range(len(foldername))#
...
def process_fit(k):
    ...
    def fit_func(x, bcr1, bcr2, mu1, sigma, tau, x00, zeta0):
        ...
        wl=np.linspace(mu1-3.5*sigma+sigma*tau*2e-1, mu1+3.5*sigma+sigma*tau*2e-1,10000)
        a = rho(wl,mu1, sigma,tau)/sum(rho(wl,mu1, sigma,tau))
        spl = UnivariateSpline(wl, a, k=4, s=0)
        I=spl.antiderivative()(wl)
        y=np.arange(I[I==np.amin(I)][0],I[I==np.amax(I)][0]+wlp,  wlp)
        xp=np.zeros(len(y))
        for i in range(len(y)):
            aus =abs(spl.antiderivative()(wl)-y[i])
            xp[i]=wl[aus==np.amin(aus)]
        wl=xp.copy()
        a=rho(xp,mu1, sigma, tau)/sum(rho(xp,mu1, sigma, tau))
        ...
        return aaa
    P0= np.zeros(7) #fit_res[0] # np.zeros(7) # [*fit_res[0],0  # fit_res[0] # [*fit_res[0,:-1],0,0]  # fit_res[0] #  [8, 2,0, 2.01e-3, pi,0, 75, 1000, 0.0004] #    [5,0,2.6e-3] # 
    P0[0]=7
    P0[1]=1.4
    P0[2]=3.7e-3
    P0[3]=5e-5
    ...
    P0[6]=0
    if (fitting):
        B=([5, 0.8, 2e-3, 1e-5, -10, -0.0005/rad, -2],[7.5, 2, 3.9e-3, 3e-4, 10, 0.0005/rad, 2])
        ...
        if (save_fit_res):
            with open(data_analysis+foldername[k]+'_fit_results_symm.mpa', 'w') as f:
...
if (plotting):
    for k in krange:
        ...
        data_analysis = sorted_fold_path+foldername[k]+"/Data Analysis/"
        diff_eff =  np.loadtxt(data_analysis+foldername[k]+'_diff_eff.mpa',skiprows=1)
        fit_res =  np.loadtxt(data_analysis+foldername[k]+'_fit_results_symm.mpa',skiprows=1)
        diff_eff_aus=diff_eff[:,2::2].copy()
        diff_eff_aus_err=diff_eff[:,3::2].copy()
        diff_eff_aus[diff_eff_aus==0]=1
        p=fit_res[0]
        ...
        if(extended_plot):
            ...
            sigma=fit_res[0,3]
            tau=fit_res[0,4]
            wl=np.linspace(mu-3.5*sigma+sigma*tau*2e-1, mu+3.5*sigma+sigma*tau*2e-1,10000)
            a = rho(wl,mu, sigma, tau)/sum(rho(wl,mu, sigma, tau))
            print(wl[a==np.amax(a)])
            ax[-1].plot(wl,a/np.amax(a), label= "WL distribution")
            ax[-1].vlines(wl[a==np.amax(a)][0], 0,1, ls="dashed", label="$\lambda_{max}=$"+str("%.3f" % (wl[a==np.amax(a)][0]*1e3),)+" nm")
...
# fit_res =  np.loadtxt(data_analysis+foldername[0]+'_fit_results_symm.mpa',skiprows=1)
...
# with open(sorted_fold_path+'tot_fit_results_symm.mpa', 'w') as f:
...
# fit_res =  np.loadtxt(sorted_fold_path+'tot_fit_results_symm.mpa',skiprows=1)
...
#     #ax.plot(x,x*0,"k.")
#     #ax.set_xlim([0,0.011])
#     a=rho(x,mu, sigma, tau)/sum(rho(x,mu, sigma, tau))
