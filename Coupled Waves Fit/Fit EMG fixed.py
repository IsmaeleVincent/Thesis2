...
tau=0.0008	#+/-	147.471394720765
mu1=3.37e-3#0.004632543663155012	#+/-	5.46776175965519e-05
sigma=0.0002
##############################################################################
...
G=2*pi/LAM
bcr1=7.915 #scattering lenght x density
bcr2=1.361
bcr3=0.
n_0 =1.
phi=0
phi1=0
d0=78
krange=[1]#range(len(foldername))#[1]#[0]#[0,2,3,4,5]#range(7,len(foldername))#[0,2,3,4,5] #np.arange(len(foldername))#
...
def process_fit(k):
    ...
    #     fit_res =  np.loadtxt(data_analysis1+foldername[1]+'_fit_results_EMG_fix.mpa',skiprows=1)
    # if k<1 or (k>1 and k<1):
    #     data_analysis1 = sorted_fold_path+foldername[2]+"/Data Analysis/"
    #     fit_res =  np.loadtxt(data_analysis1+foldername[2]+'_fit_results_EMG_fix.mpa',skiprows=1)
    ...
    def fit_func(x, bcr1, bcr2, bcr3, x00, zeta0):
        # tau=M-mu1
        ...
        th=[x[0]*rad-3*div,x[0]*rad-2*div,x[0]*rad-div,*x*rad,x[-1]*rad+div,x[-1]*rad+2*div,x[-1]*rad+3*div]#np.linspace(x[0]*rad-3*div,x[-1]*rad+3*div,2*len(x))#np.linspace(x[0]*rad-3*div,x[-1]*rad+3*div,3*len(x))#
        ...
        return aaa
    P0=  np.zeros(5) # fit_res[0,2:] #[*fit_res[0],0  # fit_res[0] # [*fit_res[0,:-1],0,0]  # fit_res[0] #  [8, 2,0, 2.01e-3, pi,0, 75, 1000, 0.0004] #    [5,0,2.6e-3] # 
    if (fitting):
        if k==1 or k>5 or 0==0:
            ...
            P0[3]=0
            P0[4]=0
            B=([0, 0, 0, -0.0005/rad, -10],[12, 10, 10, 0.0005/rad, 10])
        ...
        if (save_fit_res):
            with open(data_analysis+foldername[k]+'_fit_results_EMG_fix.mpa', 'w') as f:
...
if (plotting):
    for k in krange:
        ...
        data_analysis = sorted_fold_path+foldername[k]+"/Data Analysis/"
        diff_eff =  np.loadtxt(data_analysis+foldername[k]+'_diff_eff_new.mpa',skiprows=1)
        fit_res =  np.loadtxt(data_analysis+foldername[k]+'_fit_results_EMG_fix.mpa',skiprows=1)
        diff_eff[:,3::2]=diff_eff[:,2::2]**0.5
        diff_eff_aus=diff_eff[:,2::2].copy()
        diff_eff_aus_err=diff_eff[:,3::2].copy()
        diff_eff_aus[diff_eff_aus==0]=1
        ...
        def plot_func(x, bcr1,bcr2,bcr3, x00,zeta0):
            lambda_par=1/tau
            sigma1=(sigma**2+tau**2)**0.5
            x=diff_eff[:,0]+x00
            d=d0/np.cos((tilt[k]+zeta0)*rad)
        ...
        p_name=["$b_c\Delta\\rho_1$", "$b_c\Delta\\rho_2$","$b_c\Delta\\rho_3$", "$x_0$","$\zeta_0$"]
        p_units=[" $\mu m^{-2}$", " $\mu m^{-2}$", " $\mu m^{-2}$", " deg", "  deg"]
        text = "Fit results"
        if(extended_plot):
            p=fit_res[0]
            fig = plt.figure(figsize=(11.69,8.27), dpi=100)#constrained_layout=True
        ...
        now2=datetime.now()
        if (save_plot):
            plt.savefig(allfitsplots+str(tilt[k])+'deg_fit_EMG_fix.pdf', format='pdf',bbox_inches='tight')
            plt.close(fig)
...
if (save_fit_tot):
    ...
    fit_res =  np.loadtxt(data_analysis+foldername[0]+'_fit_results_EMG_fix.mpa',skiprows=1)
    ...
    with open(sorted_fold_path+'tot_fit_results_EMG_fix.mpa', 'w') as f:
          np.savetxt(f,tot_res, header="tilt bcr1 bcr2 mu sigma tau x0 d", fmt="%.2f "+"%.6f "*len(fit_res[0,:]))
    with open(sorted_fold_path+'tot_fit_covariances_EMG_fix.mpa', 'w') as f:
...
# fit_res =  np.loadtxt(sorted_fold_path+'tot_fit_results_EMG_fix.mpa',skiprows=1)
# fit_cov =  np.loadtxt(sorted_fold_path+'tot_fit_covariances_EMG_fix.mpa',skiprows=1)
...
#     if kaus%4==0:
#         if k>0:
#             plt.savefig(allfitsplots+str(tilt[k-4])+"-"+str(tilt[k-1])+'deg_wl_EMG_fix.pdf', format='pdf',bbox_inches='tight')
...
#     if k==8:
#         plt.savefig(allfitsplots+str(tilt[k])+'deg_wl_EMG_fix.pdf', format='pdf',bbox_inches='tight')
"""
"""
# for k in krange:
#     data_analysis = sorted_fold_path+foldername[k]+"/Data Analysis/"
