...
def ang_gauss(x):
    ...
    return g.pdf(x)
gx = np.linspace(norm.ppf(0.001, loc=0, scale=div), norm.ppf(0.999, loc=0, scale=div), 1000)
...
G=2*pi/LAM
bcr1= 10#scattering lenght x density
bcr2= 3
bcr3=0
n_0 =1.
phi=1.5*pi
...
tilt=np.linspace(0,80,100)#np.sort(tilt)#np.linspace(0,81,81)#
...
#     th=[x[0]-3*div,*x,x[-1]+3*div]#np.linspace(x[0]*rad-3*div,x[-1]*rad+3*div,2*len(x))#np.linspace(x[0]*rad-3*div,x[-1]*rad+3*div,3*len(x))#
...
#         eta_ang[i,:]=f_int(x)
#     # fig, ax = ax.subplots(3,figsize=(10,10))
#     # ax[0].plot(x,eta_ang[n_diff,:],"-k", label="Fit")
#     # for i in range(1,3):
#     #     ax[i].plot(x,eta_ang[n_diff-i,:],"-k", label="Fit (-"+str(i)+")")  
#     #     ax[i].plot(x,eta_ang[n_diff+i,:],"--", label="Fit (+"+str(i)+")") 
#     pendol[k,1]=np.amax(eta_ang[n_diff-1,:])
#     pendol[k,2]=np.amax(eta_ang[n_diff-2,:])
# pendel=np.loadtxt("/home/aaa/Desktop/Thesis2/Coupled Waves Fit/Pendol.txt")
# ax.plot(pendel[:,0],pendel[:,1],"--k", label="theory")
# ax.plot(pendel[:,0],pendel[:,2],"--", color=(0.8,0,0))
# with open('Pendol2.txt', 'w') as f:
#     np.savetxt(f,pendol, header="bcr1 bcr2 mu phi thickness", fmt="%.6f")
...
for k in krange:
    ...
    print(diff_eff[:,0][diff_eff_fit[:,2]==np.amax(diff_eff_fit[:,2])],diff_eff[:,0][diff_eff_fit[:,1]==np.amax(diff_eff_fit[:,1])])
...
pendol=pendol[np.argsort(pendol[:,0])]
...
plt.savefig('Pendell (no theory).pdf', format='pdf', bbox_inches='tight')
