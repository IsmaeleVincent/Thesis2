...
def process_fit(k):
    ...
    def fit_func(x, bcr1, bcr2, mu1, sigma, tau, x00, zeta0):
        ...
        wl=exponnorm.ppf(np.linspace(0.1,0.9,int(1e2)),K=tau, loc=mu1, scale=sigma)
    ...
    if (fitting):
        ...
        if (save_fit_res):
            with open(data_analysis+foldername[k]+'_fit_results_new_diff.mpa', 'w') as f:
...
if (plotting):
    for k in krange:
        ...
        def plot_func(x, bcr1, bcr2, mu1, sigma, tau, x00,zeta0):
            x=diff_eff[:,0]+x00
            d=d0/np.cos((tilt[k]+zeta0)*rad)
            wl=exponnorm.ppf(np.linspace(0.01,0.99,int(1e2)),K=tau, loc=mu1, scale=sigma)
            a=rho(wl,tau, mu1, sigma)/sum(rho(wl,tau, mu1, sigma))
            th=[x[0]*rad-3*div,*x*rad,x[-1]*rad+3*div]#np.linspace(x[0]*rad-3*div,x[-1]*rad+3*div,2*len(x))#np.linspace(x[0]*rad-3*div,x[-1]*rad+3*div,3*len(x))#
            S=np.zeros((2*n_diff+1,len(th)),dtype=complex)
            eta=S.copy().real
        ...
        if(extended_plot):
            ...
            sigma=fit_res[0,3]
            tau=fit_res[0,4]
            wl=exponnorm.ppf(np.linspace(0.01,0.99,1000),K=tau, loc=mu, scale=sigma)  
...
fit_res =  np.loadtxt(data_analysis+foldername[0]+'_fit_results_new_diff.mpa',skiprows=1)
...
for k in krange:
    ...
    sigma=fit_res[0,3]
    tau=fit_res[0,4]
    wl=exponnorm.ppf(np.linspace(0.01,0.99,int(1e2)),K=tau, loc=mu, scale=sigma)
