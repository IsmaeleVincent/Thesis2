...
fit_plots = fit_of_fits+"Fit plots/"
...
##############################################################################
for meas in [0]:#range(len(foldername)):
    ...
    if (plotting):
        def process_plot(k):
            ...
            if(extended_plot):
                ...
                now2=datetime.now()
                plt.savefig(fit_plots+str(tilt[k])+'deg_fit_bcr_fix_'+str(tilt[meas])+'_deg.png', format='png',bbox_inches='tight')
                plt.close(fig)
