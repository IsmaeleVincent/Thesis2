...
foldername=foldername[np.argsort(tilt)]
for name in names:
    tot_fit_res = np.loadtxt(sorted_fold_path+"Total results 9 sep/tot_fit_results_"+name+".mpa",skiprows=1)
    tot_fit_cov =  np.loadtxt(sorted_fold_path+"Total results 9 sep/tot_fit_covariances_"+name+".mpa",skiprows=1)
    for k in krange:
        ...
        with open(data_analysis+foldername[k]+"_fit_results_"+name+".mpa", "w") as f:
            np.savetxt(f,(p,cov), header="bcr1 bcr2 mu sigma tau x_0 zeta_0", fmt="%.6f")
