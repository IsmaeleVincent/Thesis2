...
def fit_func(x, bcr1, bcr2, mu, x00,d):
    x=diff_eff[:,0]+x00
    d=d/np.cos(45*rad)
...
P0= np.zeros(5) # [*fit_res[0,:-1],0,0]  # fit_res[0] #  [8, 2,0, 2.01e-3, pi,0, 75, 1000, 0.0004] #    [5,0,2.6e-3] # 
P0[0]=5
P0[1]=3
P0[2]=2e-3
P0[3]=0
...
# P0[9]=0.0
if (fitting):
    B=([0, 0, 1.8e-3, -0.05/rad, 7],[15, 15, 2.5e-3, 0.05/rad, 11])
    ...
    if (save_fit_res):
        with open(data_analysis+fold_name+'_fit_results_mono.mpa', 'w') as f:
...
if (plotting):
    ...
    print(fold_name)
    diff_eff =  np.loadtxt(data_analysis+fold_name+'_diff_eff.mpa',skiprows=1)
    fit_res =  np.loadtxt(data_analysis+fold_name+'_fit_results_mono.mpa',skiprows=1)
    p=fit_res[0]
    #print(p)
    for i in range(len(diff_eff[:,0])): 
        s=sum(diff_eff[i,2::2])
    ...
    for i in range(1,3):
        ...
        diff_eff_fit[2+i,:]=diff_eff[:,6+2*i].copy()
    def plot_func(x, bcr1, bcr2, mu, x00,d,L):
        d=d/np.cos(45*rad)
        x=diff_eff[:,0]+x00
        th=[x[0]*rad-3*div,*x*rad,x[-1]*rad+3*div]
        S=np.zeros((2*n_diff+1,len(th)),dtype=complex)
    ...
    # print(wlpoints)
    thx=diff_eff[:,0]*rad
    eta=plot_func(diff_eff[:,0], *p,1e8)
    eta1=plot_func(diff_eff[:,0], 15.54,1.98, 2e-3,-0.021,44,40)
    chi=chisquare((np.transpose(diff_eff[:,2::2])).ravel(),eta.ravel())
    #print("here")
    # bbb=eta.ravel()
    # plt.plot(bbb)
    ...
    ax[0].text(diff_eff[0,0]*rad,np.amin(diff_eff_fit[2,:])-0.02, text,  bbox=dict(boxstyle="square", ec=(0, 0, 0), fc=(1,1,1)))
