...
mu0= 2.3e-3 #2.5e-3#0.004632543663155012	#+/-	5.46776175965519e-05
sigma0=0.2e-3
# M = mu0+tau0
...
def fit_func(x, bcr1, bcr2, sigma, x00,d):
    x=diff_eff[:,0]+x00
    d=d/np.cos(45*rad)
    # d=d0/np.cos((tilt[k]+zeta0)*rad)
    wl=np.linspace(mu-3*sigma, mu+3*sigma, 10000)
    ...
    wl=xp.copy()
    a=rho(xp, mu, sigma)/sum(rho(xp, mu, sigma))
...
# P0[9]=0.0
if (fitting):
    B=([0, 0, 1e-6, -0.005/rad, 7],[15, 15, 5e-4, 0.005/rad, 11])
    ...
    if (save_fit_res):
        with open(data_analysis+fold_name+'_fit_results.mpa', 'w') as f:
...
if (plotting):
    ...
    for i in range(1,3):
        ...
        diff_eff_fit[2+i,:]=diff_eff[:,6+2*i].copy()
    def plot_func(x, bcr1, bcr2, sigma, x00,d):
        # tau=M-mu
    ...
    for i in range(1,n_diff+1):
        ...
        ax[i].legend()
    p_name=["$(b_c \\rho)_1$","$(b_c \\rho)_2$","$\\sigma$", "$x_0$","d"]
    p_units=[" $1/\mu m^2$"," $1/\mu m^2$"," nm", " deg"," $\mu m$"]
    text = "Fit results"
    print(text)
    fit_res[0,2]*=1e3
    fit_res[1,2]*=1e3
